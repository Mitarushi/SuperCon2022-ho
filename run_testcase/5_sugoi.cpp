...
// neighbor_search の result2 で呼び出す construction で priority を 100 から 5 にジョジョに下げる手法
#include <bits/stdc++.h>
using namespace std;
...
std::vector<int> neighbor_search(std::vector<std::vector<char>> &count, std::vector<int> &from, int priority, int restriction, int magnitude, int iter) {
    ...
    for (int iter = 0; iter < 100; iter++) {
        ...
        std::vector<int> result2 = construction(uncover, count, priority2, restriction);
...
pair<int, std::vector<int>> set_cover(int p, std::vector<std::vector<int>> &index) {
    ...
    for (int iter = 0; iter < 150; iter++) {
        ...
        if (x_score < optimal_score * (1 + (double)restriction / 100.0)) {
            x = neighbor_search(count, x, priority, restriction, 30, iter);
